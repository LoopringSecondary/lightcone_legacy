...
class MarketManagerRecoverySpec
  ...
  feature("test recovery") {
    scenario("market manager recovery") {
      ...
      Then("mock order2 filled half amounts")
      /*
       * mock filled amount
       * */
      ...
      addFilledAmountExpects({
        case req =>
          GetFilledAmount.Res(
            req.orderIds.map { id =>
              val filled: Amount =
                if (id == order2.hash) amountS else BigInt(0)
              id -> filled
            }.toMap
          )
      })

      Then("send filled amount event")

      actors.get(MultiAccountManagerActor.name) ! OrderFilledEvent(
      ...
      actors
        .get(MultiAccountManagerActor.name) ! AddressAllowanceUpdatedEvent(
        address = account1.getAddress,
        token = dynamicBaseToken.getAddress(),
        allowance = Some(
          "1000"
