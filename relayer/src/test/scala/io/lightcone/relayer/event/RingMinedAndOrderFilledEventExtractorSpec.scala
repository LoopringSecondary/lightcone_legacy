  ...
  "ethereum event extractor actor test" must {
    "correctly extract all kinds events from ethereum blocks" in {
      ...
      val order2 = createRawOrder(
        ...
        amountS = BigInt(order1.amountB.toByteArray))(account2)
      Await.result(
      ...
      (BigInt(weth_ba1_2.balance.toByteArray) - BigInt(
        weth_ba1_1.balance.toByteArray)).toString() should be("1" + "0" * WETH_TOKEN.decimals)

      (BigInt(lrc_ba1_1.balance.toByteArray) - BigInt(
        lrc_ba1_2.balance.toByteArray)).toString() should be("13" + "0" * LRC_TOKEN.decimals)

      (BigInt(lrc_ba1_1.allowance.toByteArray) - BigInt(
        lrc_ba1_2.allowance.toByteArray)).toString() should be("13" + "0" * LRC_TOKEN.decimals)

      (BigInt(lrc_ba2_2.balance.toByteArray) - BigInt(
        lrc_ba2_1.balance.toByteArray)).toString() should be("7" + "0" * LRC_TOKEN.decimals)

      println("---------" + weth_ba2_1.balance)
      println("---------" + weth_ba2_2.balance)

      (BigInt(weth_ba2_1.balance.toByteArray) - BigInt(
        weth_ba2_2.balance.toByteArray)).toString() should be("1" + "0" * WETH_TOKEN.decimals)
