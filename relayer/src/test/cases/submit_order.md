...
--- 
分为5种情况：无余额无授权、有余额但是无授权，有余额有授权但是授权不足，有充足的余额和授权，重复提交订单，连续下单
...
1. 有余额授权也足够
    ...
    ---
1. 有足够余额授权但是余额不足时
    - **Objective**：有足够的余额和授权，但是余额不足
    - **测试设置**：
        1. 订单余额设置为100LRC，授权为100
        2. 提交订单，sell：400LRC， fee:100LRC
    - **结果验证**：
        1. **返回结果**：提交成功
        1. **读取我的订单**：该订单状态为：PENDING
        1. **读取市场深度**：为80
        1. **读取我的成交**：为空
        1. **读取市场成交**：为空
        1. **读取我的账号**：`balance=100,allowance=100,avaliableBalance=0,avaliableAllowance=0`
    - **状态**: Planned
    - **拥有者**: 红雨
    - **其他信息**：NA    

    ---
1. 有足够余额授权但是fee的金额不足时
    - **Objective**：有足够的余额和授权，但是作为fee的token的金额不足时，应当提交成功但是规模相应缩小
    - **测试设置**：
        1. 订单余额设置为100LRC和10GTO，授权为100LRC和10GTO
        2. 提交订单，sell：100LRC， fee:20GTO
    - **结果验证**：
        1. **返回结果**：提交成功
        1. **读取我的订单**：该订单状态为：PENDING
        1. **读取市场深度**：为50
        1. **读取我的成交**：为空
        1. **读取市场成交**：为空
        1. **读取我的账号**：LRC：`balance=100,allowance=100,avaliableBalance=50,avaliableAllowance=50`
        GTO：`balance=10,allowance=10,avaliableBalance=0,avaliableAllowance=0`
    - **状态**: Planned
    - **拥有者**: 红雨
    - **其他信息**：NA
	...
	---
1. 重复提交订单
    - **Objective**：设置新账户有足够的余额授权，提交一笔订单，应该提交成功，再次提交该笔订单应该失败
    - **测试设置和验证**：
        1. 设置新账户，订单余额设置为1000LRC，授权为1000
		...
		2. 继续提交该订单 => 
		 	- **验证**：
		        1. **返回结果**：提交失败
		        1. **读取我的订单**：getOrders返回一条数据，该条订单的状态为：PENDING
		        1. **读取市场深度**：为100
		        1. **读取我的成交**：为空
		        1. **读取市场成交**：为空
		        1. **读取我的账号**：`balance=1000,allowance=1000,avaliableBalance=880,avaliableAllowance=880`
...
1. 发起过取消订单的Ethereum交易
    ...
    - **结果验证**：
        1. **返回结果**：提交失败，`SubmitOrder.Res.status == false`
        1. **读取我的订单**：getOrders返回了该订单，但是状态为CANCELLED  (？？ 这地方是否是写入了数据库)
        1. **读取市场深度**：为空
        1. **读取我的成交**：为空
        1. **读取市场成交**：为空
