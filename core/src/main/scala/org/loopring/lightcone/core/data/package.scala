...
package object data {
  ...
  implicit class RichOrderRing(raw: OrderRing) {
    ...
    def middleRate(feeToken: String): Double = {
      val makerSellPrice =
        (raw.maker.order.amountS / raw.maker.order.amountB).doubleValue

      val takerSellPrice =
        (raw.taker.order.amountS / raw.taker.order.amountB).doubleValue
      ...
      if (raw.maker.order.amountS =~ feeToken) priceByMaker
      else 1 / priceByMaker
    }
