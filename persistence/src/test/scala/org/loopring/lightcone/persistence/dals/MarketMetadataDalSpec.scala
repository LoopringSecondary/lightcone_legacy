  ...
  "save markets config" must "save some markets config" in {
    ...
    val markets = Seq(
      MarketMetadata(
        ...
        marketHash = MarketId(WETH, LRC).keyHex()
      ),
      MarketMetadata(
        ...
        marketHash = MarketId(WETH, BNB).keyHex()
      ),
      MarketMetadata(
        ...
        marketHash = MarketId(WETH, ZRX).keyHex()
      )
    )
      ...
      case m: MarketMetadata if m.secondaryTokenSymbol == "LRC" =>
        assert(
          m.status == MarketMetadata.Status.ENABLED
            ...
            && m.marketHash == MarketId(WETH, LRC).keyHex()
        )
      case m: MarketMetadata if m.secondaryTokenSymbol == "BNB" =>
        assert(
          m.status == MarketMetadata.Status.DISABLED
            ...
            && m.marketHash == MarketId(WETH, BNB).keyHex()
        )
      case m: MarketMetadata if m.secondaryTokenSymbol == "ZRX" =>
        assert(
          m.status == MarketMetadata.Status.READONLY
            ...
            && m.marketHash == MarketId(WETH, ZRX).keyHex()
        )
      case _ => assert(false)
    }
    val lrcWeth =
    ...
    assert(
      bnb1.status == MarketMetadata.Status.ENABLED
        ...
        && bnb1.marketHash == MarketId(WETH, BNB).keyHex()
    )
  }
}
