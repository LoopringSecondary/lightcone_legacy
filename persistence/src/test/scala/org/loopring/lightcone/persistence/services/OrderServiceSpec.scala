...
class OrderServiceSpec extends ServiceSpec[OrderService] {
  ...
  private def testSave(
    ...
    tokenB: String,
    validSince: Int,
    validUntil: Int): Future[Either[XRawOrder, XErrorCode]] = {
  ...
  private def testSaves(
    ...
    tokenB: String,
    validSince: Int,
    validUntil: Int): Future[Set[Either[XRawOrder, XErrorCode]]] = {
    for {
  ...
  "getOrders" must "get some orders with many query parameters" in {
    ...
    val result = for {
      ...
      queryMarket ← service.getOrders(
        ...
        Some(XSort.ASC),
        None)
      count ← service.countOrdersForRecover(
        Set.empty,
        Set.empty,
        Set.empty,
        Set.empty,
    ...
    val res = Await.result(
      result.mapTo[(Seq[XRawOrder], Seq[XRawOrder], Seq[XRawOrder], Seq[XRawOrder], Int)],
      5.second)
  ...
  "getOrdersForUser" must "get some orders with many query parameters" in {
    ...
    val res =
      Await.result(result.mapTo[(Seq[XRawOrder], Seq[XRawOrder])], 5.second)
  ...
  "countOrders" must "get orders count with many query parameters" in {
    ...
    val result = for {
      _ ← testSaves(
        ...
        validSince,
        validUntil.toInt)
      query ← service.countOrdersForRecover(
        Set(XOrderStatus.STATUS_NEW),
        owners,
        Set(tokenS),
        Set(tokenB),
  ...
  "markOrderSoftCancelled" must "soft cancel some orders with hash" in {
    ...
    val result = for {
      _ ← testSaves(
        ...
        validSince,
        validUntil.toInt)
      query1 ← service.countOrdersForRecover(
        Set(XOrderStatus.STATUS_NEW),
        owners,
        Set.empty,
        Set.empty,
      ...
      update ← service.markOrderSoftCancelled(
        Seq("0x-softcancel-02", "0x-softcancel-03", "0x-softcancel-04"))
      query2 ← service.countOrdersForRecover(
        Set(XOrderStatus.STATUS_NEW),
        owners,
        Set.empty,
        Set.empty,
        Set.empty)
    } yield (update, query1, query2)
    val res = Await.result(
    ...
    val x = res._1.length === 3 && !res._1.exists(
      _.error !== XErrorCode.ERR_NONE) && res._2 === 6 && res._3 === 3
    x should be(true)
  }
}
