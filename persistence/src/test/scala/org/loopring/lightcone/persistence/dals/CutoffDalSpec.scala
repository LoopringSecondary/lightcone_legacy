...
class CutoffDalSpec extends DalSpec[CutoffDal] {
  ...
  "saveCutoff" must "save some cutoffs" in {
    ...
    val result = for {
      ...
      queryBroker ← dal.getCutoffs(Some(XCutoff.XType.BROKER), Some(XCutoffBy(XCutoffBy.Value.Broker("0x-savecutoff-broker4"))), None)
      queryBrokerPair ← dal.getCutoffs(Some(XCutoff.XType.BROKER), Some(XCutoffBy(XCutoffBy.Value.Broker("0x-savecutoff-broker8"))), Some("XYZ-WETH"))
      queryOwner ← dal.getCutoffs(Some(XCutoff.XType.OWNER), Some(XCutoffBy(XCutoffBy.Value.Owner("0x-savecutoff-owner9"))))
    ...
    val res = Await.result(result.mapTo[(Seq[XCutoff], Seq[XCutoff], Seq[XCutoff], Seq[XCutoff], Seq[XCutoff])], 5.second)
    val x = res._1.length === 16 && res._2.length === 1 && res._3.length === 0 && res._4.length === 1 && res._5.length === 8
    x should be(true)
  }
  ...
  "hasCutoff" must "has some cutoffs" in {
    ...
    val result = for {
      ...
      queryBrokerTime ← dal.hasCutoffs(Some(XCutoff.XType.BROKER), None, None, Some(timeProvider.getTimeSeconds() - 100))
      queryBrokerPair ← dal.hasCutoffs(Some(XCutoff.XType.BROKER), Some(XCutoffBy(XCutoffBy.Value.Broker("0x-hasCutoff-broker8"))),
        Some("LRC-WETH"), Some(timeProvider.getTimeSeconds() - 100))
      queryOwner ← dal.hasCutoffs(Some(XCutoff.XType.OWNER), Some(XCutoffBy(XCutoffBy.Value.Owner("0x-hasCutoff-owner9"))), None, None)
