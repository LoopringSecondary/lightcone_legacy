...
class AddressTable(tag: Tag)
    extends BaseTable[AddressData](tag, "T_ADDRESSES") {
  ...
  def * =
    (address, balance, numTx, creatorAddress, creatorTx, updatedAtBlock) <> ((AddressData.apply _).tupled, AddressData.unapply)
}
