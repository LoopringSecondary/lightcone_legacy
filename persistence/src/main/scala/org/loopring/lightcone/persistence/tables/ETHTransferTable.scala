...
class ETHTransferTable(tableNum: Int)(tag: Tag)
    extends BaseTable[TransferData](
      tag,
      s"ETH_TRANSFER_$tableNum"
    ) {
  ...
  implicit val transferColumnType = transferEventColumnType()
  ...
  def serialized = column[Option[TransferEvent]]("serialized")
  ...
  def * =
    (
      ...
      createdAt,
      serialized
    ) <> ((TransferData.apply _).tupled, TransferData.unapply)
}
