...
import org.loopring.lightcone.proto.market_cap.{ GetTokenTickerInfoRes, TokenTickerInfo }
...
class TokenTickerInfoServiceImpl @Inject() (
  ...
  override def saveOrUpdate(tokenTicker: TokenTickerInfo) = {
    saveOrUpdate(tokenTicker)
  }

  override def queryTokenTicker(market: String): Future[GetTokenTickerInfoRes] = {
    sql"""select
             token_id,
             token_name,
             symbol,
  ...
  override def batchSaveOrUpdate(seq: Seq[TokenTickerInfo]) = {
    saveOrUpdate(seq: _*)
  }
}
