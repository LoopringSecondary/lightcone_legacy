...
package org.loopring.lightcone.gateway_bak.jsonrpc
...
private[jsonrpc] object JsonRpcProxy {
  ...
  def invoke(
    ...
    val paramTpes =
      methodMirror.symbol.paramLists.flatMap(_.map(_.typeSignature))
    ...
    val responseAny = paramTpes match {
        ...
        request.params match {
          case arr: JArray =>
            if (head <:< typeOf[Iterable[_]]) {
              ...
              arr.children match {
                case (a0: JValue) :: Nil =>
                  methodMirror(convertString2Parameter(a0, head))
                case _ =>
                  throw JsonRpcInternalException(
                    s"params not match Array",
                    id = Some(request.id)
                  )
              }
            }
      ...
      case _ =>
        throw JsonRpcInternalException(
          s"method:${methodName} has multiple parameter",
          id = Some(request.id)
        )
    }
    ...
    (responseAny match {
      case f: Future[_] =>
        f.map(resp => JsonRpcResponse(id = Some(request.id), result = resp))
      case x =>
        Future.successful(JsonRpcResponse(id = Some(request.id), result = x))
    })

  }

  def convertString2Parameter(
      jValue: JValue,
      tpe: Type
    ) = {
