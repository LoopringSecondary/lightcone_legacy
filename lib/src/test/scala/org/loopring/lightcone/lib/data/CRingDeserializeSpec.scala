...
class CRingDeserializeSpec extends FlatSpec with Matchers {

  val one = BigInt("1000000000000000000")

  "deserialize" should "deserialize ring to orders" in {
  ...
  private def compare(src: COrder, dst: COrder): Boolean = {
    (src.owner == dst.owner) &&
      (src.tokenS == dst.tokenS) &&
      (src.tokenB == dst.tokenB) &&
      (src.amountS == dst.amountS) &&
      (src.amountB == dst.amountB) &&
      (src.feeAmount == dst.feeAmount) &&
      (src.feeToken == dst.feeToken) &&
      (src.dualAuthAddress == dst.dualAuthAddress) &&
      ...
      (src.tokenReceipt == dst.tokenReceipt) &&
      (src.sig == dst.sig) &&
