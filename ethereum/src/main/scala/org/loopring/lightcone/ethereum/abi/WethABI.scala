...
class WethABI(abiJson: String) extends ERC20ABI(abiJson) {
  ...
  private[ethereum] def assembleDepositFunction(from: String, value: BigInt) =
    Deposit(from, value)

  private[ethereum] def assembleWithdrawalFunction(list: Seq[Any], from: String) = {
  ...
  private[ethereum] def assembleDepositEvent(list: Seq[Any]): Deposit = {
  ...
  private[ethereum] def assembleWithdrawalEvent(list: Seq[Any]): Withdrawal = {
