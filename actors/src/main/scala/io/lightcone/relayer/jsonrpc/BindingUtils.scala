...
class Method(
  ...
  def accepts[A <: Proto[A]: TypeTag](
    implicit pa: ProtoC[A],
    ca: ClassTag[A]) = new Accept[A, A](method, identity)
...
class Accept[A <: Proto[A]: TypeTag, //
B <: Proto[B]: TypeTag](method: String,
  requestConverter: A => B)(
    implicit module: JsonRpcBinding,
    ps: ProtoSerializer) {
  private def identity[T <: Proto[T]: TypeTag](obj: T) = obj
  ...
  def replies[C <: Proto[C]: TypeTag](
    ...
    ca: ClassTag[A],
    pd: ProtoC[C],
    cd: ClassTag[C]) = new Reply[A, B, C, C](method, requestConverter, identity)
...
class Reply[A <: Proto[A]: TypeTag, //
B <: Proto[B]: TypeTag, //
C <: Proto[C]: TypeTag, //
D <: Proto[D]: TypeTag](val method: String,
  requestConverter: A => B,
  responseConverter: C => D)(
    implicit pa: ProtoC[A],
    ca: ClassTag[A],
