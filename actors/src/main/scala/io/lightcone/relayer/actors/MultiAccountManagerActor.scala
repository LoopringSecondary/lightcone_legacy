...
object MultiAccountManagerActor extends DeployedAsShardedByAddress {
  ...
  val extractShardingObject: PartialFunction[Any, String] = {
    case SubmitOrder.Req(Some(rawOrder)) if metadataManager.isMarketActiveOrReadOnly(
      MarketPair(rawOrder.tokenS, rawOrder.tokenB)) =>
      rawOrder.owner

    case ActorRecover.RecoverOrderReq(Some(raworder)) if metadataManager.isMarketActiveOrReadOnly(
      MarketPair(raworder.tokenS, raworder.tokenB)) =>
      raworder.owner

    case req: CancelOrder.Req if req.marketPair.nonEmpty && metadataManager.isMarketActiveOrReadOnly(
      req.getMarketPair) =>
      req.owner
