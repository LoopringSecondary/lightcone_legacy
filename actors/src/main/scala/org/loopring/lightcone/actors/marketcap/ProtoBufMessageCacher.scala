...
class ProtoBufMessageCacher[T <: ProtoBuf[T]](
  ...
  private[marketcap] implicit val deserializer = new ProtoBufByteStringDeserializer[T]

  private[marketcap] implicit val serializer = new ProtoBufByteStringSerializer[T]

  private[marketcap] val seqAnyByteStringDeserializer = new SeqAnyByteStringDeserializer[T]
  ...
  private[marketcap] def seq2ByteString(data: Seq[_]): ByteString = {
